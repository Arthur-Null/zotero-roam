...
import {
  createPersisterWithIDB,
  shouldQueryBePersisted,
  validateShortcuts,
} from "../../setup";
...
const QCProvider: FC<QCProviderProps> = ({ children, idbDatabase }) => {
  ...
  const persister = useMemo(
    () => createPersisterWithIDB(idbDatabase),
    [idbDatabase],
  );
  ...
  const Provider = useMemo(
    () => (cacheEnabled ? PersistQueryClientProvider : QueryClientProvider),
    [cacheEnabled],
  );

  return (
    <Provider client={queryClient} {...persisterProps}>
      {children}
    </Provider>
  );
};
...
const AppWrapper = (props: AppWrapperProps) => {
  ...
  const sanitizedShortcuts = useMemo(
...
class App extends Component<AppProps, AppState> {
  ...
  componentDidUpdate(prevProps) {
    if (
      this.props.requests.dataRequests.length == 0 &&
      prevProps.requests.dataRequests.length > 0
    ) {
      this.setState((_prev) => {
        queryClient.clear();
        return {
  ...
  toggleExtension() {
    this.setState((prevState) => {
      ...
      return {
        status:
          status == ExtensionStatusEnum.OFF
            ? ExtensionStatusEnum.ON
            : ExtensionStatusEnum.OFF,
      };
    });
...
export { AppWrapper, ExtensionContext, useExtensionContext, queryClient };
